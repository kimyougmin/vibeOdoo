...
export async function GET(request: Request) {
  try {
    ...
    console.log('XML-RPC 연차/휴가 조회 시작');
    ...
    const holidays = await new Promise<any[]>((resolve, reject) => {
      models.methodCall(
        'execute_kw',
        [
          DB, uid, PASS,
          'hr.leave', 'search_read',
          [domain, ['id', 'name', 'employee_id', 'holiday_status_id', 'date_from', 'date_to', 'number_of_days', 'state'], null, limit]
        ],
        (err: any, value: any) => {
          if (err) {
            console.error('연차/휴가 조회 실패:', err);
            reject(err);
          } else {
            console.log('연차/휴가 조회 성공:', value);
            resolve(value || []);
          }
        }
      );
    ...
    return NextResponse.json(
      { error: '연차/휴가 데이터 조회 실패', details: (error as Error).message }, 
      { status: 500 }
    );
  }
} 